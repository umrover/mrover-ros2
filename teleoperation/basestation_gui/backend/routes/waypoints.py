...
from backend.models_pydantic import (
    BasicWaypoint,
    BasicWaypointList,
    AutonWaypointList,
    CreateAutonWaypoint,
...
def add_to_store(data: CreateAutonWaypoint):
    ...
    try:
        ...
        conn.commit()
        return {"status": "success", "waypoint": fetch_store_row(conn, cursor.lastrowid)}
    finally:
        if conn:
            conn.close()
...
def update_store(waypoint_id: int, data: UpdateAutonWaypoint):
    ...
    try:
        ...
        if not wp["deletable"]:
            ...
            if immutable:
                raise HTTPException(
                    status_code=403, detail=f"Cannot modify {', '.join(immutable)} on permanent waypoints"
                )
