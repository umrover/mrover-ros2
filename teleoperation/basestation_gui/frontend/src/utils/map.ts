...
const convertDMS = function (coordinate: DMS, format: string): DMS {
  ...
  const coordinate_total: number =
    coordinate.d + coordinate.m / 60 + coordinate.s / 3600
  const result = {
    d: 0,
    m: 0,
    s: 0,
  ...
  switch (format) {
    case 'DMS':
      ...
      result.m = Math.trunc(
        Number(((coordinate_total - result.d) * 60).toFixed(MIN_DECIMALS)),
      )
      result.s = +(
        ((coordinate_total - result.d) * 60 - result.m) *
        60
