...
CameraConfigWidget::CameraConfigWidget(QWidget* parent) : QWidget(parent) {
    ...
    mOpenNewWindowButton = new QPushButton(tr("Open New Window"), this);

    mMainLayout->addWidget(mArmButton);
    mMainLayout->addWidget(mScienceButton);
    mMainLayout->addWidget(mAutonButton);
    mMainLayout->addWidget(mOpenNewWindowButton);
    ...
    connect(mOpenNewWindowButton, &QPushButton::clicked, this, &CameraConfigWidget::onOpenNewWindowClicked);
...
void CameraConfigWidget::onOpenNewWindowClicked() {
    emit openNewWindowSignal();
}
