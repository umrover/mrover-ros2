...
test('autonomy navstate status color', async ({ page }) => {
  const base = 'http://localhost:8080/AutonTask';
  await page.goto(base);

  // Helper: try selectors and click the first that exists
  const navStateHeader = page.locator('h2:text("Nav State: OffState")');
  await expect(navStateBar).toBeVisible();
  const bgColor = await navStateBar.evaluate(el => {
    const style = window.getComputedStyle(el);
    if(style.bgColor != 'rgba(0,0,0,0)'){
      return style.bgColor;
    }
    return window.getComputedStyle(el.parentElement).bgColor;
  });
  expect(backgroundColor).toBe('rgb(220, 53, 69)');
