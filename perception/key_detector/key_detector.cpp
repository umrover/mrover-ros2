...
namespace mrover{
	KeyDetector::KeyDetector(rclcpp::NodeOptions const& options) : rclcpp::Node(NODE_NAME, options), mFSMTimer{create_wall_timer(std::chrono::milliseconds(1000), [&](){updateFSM();})}, fsm_ctx(std::make_shared<FSMCtx>()), mStateMachine{"Key Detector FSM", StateMachine::make_state<TargetKey>(fsm_ctx)}, mStatePublisher{this, mStateMachine, "key_detector_fsm_structure", 10, "key_detector_fsm_state", 10}{
