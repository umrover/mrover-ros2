...
class AStar:
    ...
    def use_astar(self, dest: np.ndarray) -> bool:
        rover_in_map = self.context.rover.get_pose_in_map()

        # If no rover pose, no trajectory, or not enough points in star_traj, skip
        if rover_in_map is None:
            return False

        straight_path = segment_path(context=self.context, dest=dest[:2], seg_len=self.context.env.cost_map.resolution)
        for point in straight_path.coordinates:
            if is_high_cost_point(point=point, context=self.context):
                return True
        return False

    def generate_trajectory(self, dest: np.ndarray) -> Trajectory:
        """
        ...
        """
        start_time = self.context.node.get_clock().now()
        rover_SE3 = self.context.rover.get_pose_in_map()
        if rover_SE3 is None:
            self.context.node.get_logger().warn("Rover has no pose, cannot astar...")
        ...
        dest_ij = cartesian_to_ij(self.context, dest)
        ...
        self.context.rover.send_drive_command(Twist())  # Stop while planning
        ...
        try:
            ...
            self.context.node.get_logger().info("No path found")
            trajectory.reset()
            return Trajectory(np.array([]))
            # raise NoPath

        if occupancy_list is not None:
            cartesian_coords = ij_to_cartesian(self.context, np.array(occupancy_list))
        ...
        self.context.node.get_logger().info(
            f"A-Star pathplanning took {(self.context.node.get_clock().now() - start_time).nanoseconds / 10e9} seconds"
        )
