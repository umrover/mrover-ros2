...
def smoothing(
    ...
    context.publish_path_marker(output.coordinates, [1.0, 0.0, 0.0], "relaxed")

    if should_interpolate:
        # Perform spline interpolation
        spline_path = SplineInterpolation.interpolate(context, output, spacing=0.5)
        # plot_spline_path = SplineInterpolation.interpolate(context, output, spacing=0.1)

        context.publish_path_marker(output.coordinates, [0.0, 1.0, 0.0], "interp")
...
class Relaxation:
    ...
    def cost_segment(ctx: Context, trajectory: Trajectory, i: int, j: int) -> float:
        ...
        for y, x in interpolated:
            if (
                ...
                cost_arr = [
                    cost_map.data[min(y, max_y), min(x, max_x)],
                    cost_map.data[min(y + 1, max_y), min(x, max_x)],
                    cost_map.data[min(y, max_y), min(x + 1, max_x)],
                    cost_map.data[min(y + 1, max_y), min(x + 1, max_x)],
                ]
                ...
                cost += segment_interval * (
                    cost_map.data[min(int(y + 0.5), max_y), min(int(x + 0.5), max_x)] + 0.001
    ...
    def relax_single(ctx: Context, trajectory: Trajectory) -> tuple[Trajectory, float]:
        ...
        """
        total_cost, segment_costs = Relaxation.cost_full(ctx, trajectory)
