...
WebsocketServer::WebsocketServer(rclcpp::Logger const& logger, std::string_view host, std::uint16_t port, handler_t&& on_open, handler_t&& on_close)
    : m_logger{logger}, m_acceptor{m_context}, m_on_open{std::move(on_open)}, m_on_close{std::move(on_close)} {
    std::scoped_lock guard{m_mutex};

    tcp::endpoint endpoint{net::ip::make_address(host), port};
    RCLCPP_INFO_STREAM(m_logger, "Starting websocket server @" << endpoint);
    ...
    m_io_thread = std::jthread{[this] {
        RCLCPP_INFO_STREAM(m_logger, "IO service started");
        m_context.run();
        RCLCPP_INFO_STREAM(m_logger, "IO service stopped");
    }};
}
...
auto WebsocketServer::acceptAsync() -> void {
    m_acceptor.async_accept(m_context, [this](beast::error_code const& ec, tcp::socket socket) {
        ...
        if (ec) {
            RCLCPP_WARN_STREAM(m_logger, std::format("Failed to accept: {}", ec.message()));
            return;
        }
...
auto WebsocketServer::broadcast(std::span<std::byte> data) -> void {
    ...
    for (auto it = m_clients.begin(); it != m_clients.end();) {
        try {
            ...
            RCLCPP_WARN_STREAM(m_logger, std::format("Exception writing to client: {}", e.what()));
