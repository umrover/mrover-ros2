...
namespace mrover {
    ...
    private:
        Pin m_direction_pin{};
        std::optional<Pin> m_dual_direction_pin{};
        ...
        auto set_direction_pins(Percent duty_cycle) const -> void {
            ...
            if (m_dual_direction_pin) m_dual_direction_pin->write(negative_state);
        }
    ...
    public:
        ...
        explicit HBridge(TIM_HandleTypeDef* timer, std::uint32_t channel, Pin positive_pin, Pin negative_pin) :
            m_direction_pin(positive_pin), m_dual_direction_pin(negative_pin), m_timer(timer), m_channel(channel), m_max_pwm(0_percent) {
        ...
        explicit HBridge(TIM_HandleTypeDef* timer, std::uint32_t channel, Pin direction_pin) :
            m_direction_pin(direction_pin), m_timer(timer), m_channel(channel), m_max_pwm(0_percent) {
            // Prevent the motor from spinning on boot up
            __HAL_TIM_SET_COMPARE(m_timer, m_channel, 0);
            HAL_TIM_PWM_Start(m_timer, m_channel);
        }
