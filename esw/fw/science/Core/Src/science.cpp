...
namespace mrover {
    ...
    UVSensor uv_sensor1 = mrover::UVSensor(&adc_sensor1, 0);
    UVSensor uv_sensor2 = mrover::UVSensor(&adc_sensor2, 0);
//    MethaneSensor methane_sensor = mrover::MethaneSensor(&adc_sensor2, 0);
    ...
    void event_loop() {
        ...
        SensorData uv_data1 = {.id = static_cast<std::uint8_t>(ScienceDataID::METHANE), .data = 0};
        SensorData uv_data2 = {.id = static_cast<std::uint8_t>(ScienceDataID::UV), .data = 0};
...
//            uv_data.data = uv_sensor.update_uv_blocking();
            uv_data1.data = uv_sensor1.update_uv_blocking();
            uv_data2.data = uv_sensor2.update_uv_blocking();
            ...
            HAL_Delay(50);
            fdcan_bus.broadcast(uv_data1, SCIENCE_A, JETSON_ADDRESS);
            HAL_Delay(50);
            fdcan_bus.broadcast(uv_data2, SCIENCE_A, JETSON_ADDRESS);
            HAL_Delay(50);
        }
    }
