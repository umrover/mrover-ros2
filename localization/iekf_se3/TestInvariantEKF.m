...
for i = 1:num_samples
    ...
    position_dead_reckoning(i,:) = filter.X(1:3,4)';
    ...
    vel_noise_x = a + (b-a).*rand(1,1);
    vel_noise_y = a + (b-a).*rand(1,1);
    vel_noise_z = a + (b-a).*rand(1,1);

    filter.vel_predict([0 + vel_noise_x; 0 + vel_noise_y; 0 + vel_noise_z],  [vel_noise_x; vel_noise_y; vel_noise_z], dt);
...
for i = 1:num_samples
    ...
    position_filtered(i,:) = filter.X(1:3,4)';
    ...
    if (mod(i, 50) == 0)
        ...
        filter.vel_predict([0 + vel_noise_x; 0 + vel_noise_y; 0 + vel_noise_z],  [vel_noise_x; vel_noise_y; vel_noise_z], dt * 50);
    ...
    mag_radians = 3.14/2 + mag_noise_x;
