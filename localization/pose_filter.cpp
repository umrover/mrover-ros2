...
PoseFilter::PoseFilter() : Node("pose_filter") {
    ...
    imu_watchdog = this->create_wall_timer(std::chrono::seconds(IMU_WATCHDOG_TIMEOUT), [&]() {
        RCLCPP_WARN("IMU data watchdog expired");
        current_imu_calib.reset();
        current_imu_uncalib.reset();
        correction_rotation.reset();
    });
    
    correction_timer = this->create_wall_timer(WINDOW, [this]() -> void {
        correction_timer_callback();
    });

    odometry_pub = this->create_publisher<nav_msgs::msg::Odometry>("/odometry", 1);

    calibration_status_sub = this->create_subscription<mrover::msg::CalibrationStatus>("/imu/calibration", 1, [&](mrover::msg::CalibrationStatusConstPtr const& status) {
        calibration_status = *status;
    });

    twist_sub = this->create_subscription<geometry_msgs::msg::Twist>("/cmd_vel", 1, [&](mrover::msg::TwistConstPtr const& twist) {
        twists.push_back(*twist);
    });

    imu_uncalib_sub = this->create_subscription<sensor_msgs::msg::Imu>("/imu/data_raw", 1, [&](sensor_msgs::msg::ImuConstPtr const& imu) {
        current_imu_uncalib = *imu;
    });

    imu_calib_sub = this->create_subscription<sensor_msgs::msg::Imu>("/imu/data", 1, [&](sensor_msgs::msg::ImuConstPtr const& imu) {
        current_imu_calib = *imu;
    });

    pose_sub = this->create_subscription<geometry_msgs::msg::Vector3Stamped>("/linearized_position", 1, [this](geometry_msgs::msg::Vector3Stamped const& msg) -> void {
        pose_sub_callback(msg);
    });

    rover_frame = declare_parameter("rover_frame", "base_link");
    world_frame = declare_parameter("map_frame", "map");

}

void PoseFilter::pose_sub_callback(geometry_msgs::msg::Vector3StampedConstPtr const& msg) {
    ...
    if (!mag_fully_calibrated && current_imu_calib && correction_rotation) {
        ...
        RCLCPP_WARN_THROTTLE(1, "Not enough IMU data available");
        return;
    }

    SE3Conversions::pushToTfTree(tf_broadcaster, rover_frame, world_frame, pose_in_map);

    SE3d::Tangent twist;
    if (last_pose_in_map && last_pose_time) {
        twist = (pose_in_map - last_pose_in_map.value()) / (msg->header.stamp - last_pose_time.value()).toSec();
    }
...
void PoseFilter::correction_timer_callback() {
    ...
    for (auto total = static_cast<double>(twists.size()); auto const& twist : twists) {
        mean_twist.linear.x += twst.linear.x / total;
    ...
    if (mean_twsit.linear.x < MIN_LINEAR_SPEED) return;
    if (std::fabs(mean_twist.angular.z) > MAX_ANGULAR_SPEED) return;

    RCLCPP_INFO("Rover is being commanded forward");
    ...
    if (R2d mean_velocity_in_map = rover_velocity_sum / static_cast<double>(readings); mean_velocity_in_map.norm() < MIN_LINEAR_SPEED) {
        RCLCPP_INFO_STREAM(std::format("Rover is not moving fast enough: speed = {} m/s", mean_velocity_in_map.norm()));
        return;
    }

    if (rover_heading_change > MAX_ANGULAR_CHANGE) {
        RCLCPP_INFO_STREAM(std::format("Rover is not moving straight enough: heading change = {} rad", rover_heading_change));
        return;
    }
    ...
    corrected_rotation = Eigen::AngleAxisd{heading_correction_delta, R3d::UnitZ()};

    RCLCPP_INFO_STREAM(std::format("Correcting heading by: {}", correction_rotation->z()));
}
