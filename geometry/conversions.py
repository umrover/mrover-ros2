...
def from_tf_tree(tf_buffer: tf2_ros.buffer, child_frame: str, parent_frame: str, timeout: Duration = Duration()) -> SE3:
    se3, _ = from_tf_tree_with_time(tf_buffer, child_frame, parent_frame, timeout)
    return se3


def from_tf_tree_with_time(tf_buffer: tf2_ros.buffer, child_frame: str, parent_frame: str, timeout: Duration = Duration()) -> tuple[SE3, Time]:
